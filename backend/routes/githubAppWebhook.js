...
router.post('/', verifySignature, async (req, res) => {
    ...
    if (event === 'push') {
        ...
        for (const commit of commits) {
            ...
            // Use Gemini to analyze commit
            const analysis = await analyzeCommit(message);

            if (analysis && analysis.found && analysis.id && analysis.action === 'Complete') {
                console.log(`Gemini identified Task ID: ${analysis.id} as completed.`);
